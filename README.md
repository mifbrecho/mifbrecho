...
└── README.md
Fluxo principal
Admin (irmã) entra no painel → cadastra produtos com fotos → define preço e estoque.
Cliente abre o site ou o app → navega nas peças → adiciona no carrinho.
No checkout: escolhe endereço de entrega → gera Pix via Mercado Pago.
Cliente paga o Pix → webhook do Mercado Pago confirma → pedido muda para "pago".
Admin vê na fila de pedidos → organiza rota de entrega → marca como "enviado" / "entregue".
...
Cliente (Site + App)
 Listagem de produtos com filtros (categoria, preço, tamanho)
 Página de detalhe do produto
 Carrinho
 Checkout com endereço
 Geração de Pix (QR + copia e cola)
 Acompanhamento do pedido
 Histórico de compras
Admin
 Login protegido
 CRUD de produtos (com upload de fotos)
 Lista de pedidos em fila
 Status: pendente → pago → em separação → enviado → entregue
 Visualização de entregas do dia (rotas)
 Dashboard simples (vendas do dia/semana)
Automação
...
Criar projeto no Supabase e rodar todas as migrations, em ordem, da pasta supabase/migrations/ (001 até a última — são vários arquivos, não só o 001)
Configurar Mercado Pago (modo teste primeiro)
Subir o Next.js na Vercel
Buildar o app com EAS (Expo Application Services)
